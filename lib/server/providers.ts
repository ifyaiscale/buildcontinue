...
async function remote(url: string, init: RequestInit) {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      const code = payload && typeof payload === "object" && "error" in payload ? (payload as { error?: unknown }).error : "";
      if (typeof code === "string" && /^[a-z_]{1,60}$/i.test(code)) providerCode = `, code ${code}`;
    } catch { /* Provider error bodies are optional and never exposed. */ }
    throw new HttpError(502, `Provider verification failed (HTTP ${response.status}${providerCode}). Check the account identifier and try again.`);
  }
