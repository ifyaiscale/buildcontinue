...
async function whopRequest(path: string, apiKey: string, init: RequestInit = {}) {
  ...
  if (response.status === 401 || response.status === 403) {
    throw new HttpError(422, "This Whop API key needs the developer:manage_webhook permission. Update the brand's Whop connection with a key that has that permission, then retry.");
  }
...
async function provision(request: Request) {
  ...
  const endpoint = `${appOrigin.origin}/api/webhooks/whop/${brand.slug}`;

  // A webhook secret is shown only at creation time. If the dedicated Limitless
  // callback already exists but its secret was never stored, recreate only that
  // exact callback so we can immediately persist the new signing secret.
